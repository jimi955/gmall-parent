...
public class CartServiceImpl implements CartService {
    ...
    @Override
    public CartResponse addToCart(Long skuId, Integer num, String cartKey, String accessToken) throws InterruptedException, ExecutionException {
    ...
    private CartItem addItemToCart(Long skuId, Integer num, String finalCartKey) throws ExecutionException, InterruptedException {
        ...
        skuFuture.get();//在线等结果 等待
        // 检查购物车中是否已经存在这个购物项
        if (itemJson!=null && !StringUtils.isEmpty(itemJson)) {
        ...
        if (map != null && !map.isEmpty()) {
            ...
            map.entrySet().forEach((item) -> {
                ...
                if (!key.equalsIgnoreCase(CartConstant.CART_CHECKED_KEY)) {
                    ...
                    try {
                        // 存到用户账号下
                        // addItemToCart(Long skuId, Integer num, String finalCartKey)
            ...
            map.clear(); // redisson就会删除oldCartKey对应的map
        }
